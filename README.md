...
- **오늘**: 오늘 먹을 약을 **시간대별로** 묶어서 표시 + 진행 바. 카드를 터치하면 복용 완료(시각 기록), 다시 터치하면 취소
- **기록**: 월 캘린더 — 전부 복용 ●(초록) / 일부 ◐(노랑) / 미복용 ○(회색). 날짜를 누르면 상세
- **약 관리**: 약 추가/수정/삭제 (삭제해도 과거 기록은 보존). 약마다 시간대(아침/점심/저녁/자기 전/아무 때나)를 지정
- **알림**: NAS가 시간대별로 확인해서 아직 체크 안 한 약이 있을 때만 아이폰으로 푸시 → [nas/README.md](nas/README.md)
...
2. 왼쪽 메뉴 **SQL Editor** → 이 폴더의 `setup.sql` 내용 전체를 붙여넣고 **Run**
   - 이미 운영 중인 DB에 시간대 기능만 추가하는 경우에는 `migrate-slot.sql` 을 실행하세요
...
- 날짜 기준은 기기의 로컬 시간대이며, 하루에 약별 1회 체크 모델입니다.
  같은 약을 하루 두 번 먹는다면 "베르베린(아침)" / "베르베린(저녁)" 처럼 따로 등록하세요.
- 알림 시각의 실제 소스는 NAS의 작업 스케줄러입니다. `app.js` 의 `SLOTS` 에 적힌 시각은 화면 표시용이라,
  시간을 바꿀 때는 양쪽을 같이 고쳐야 합니다.
